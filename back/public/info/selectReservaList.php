...
FROM vivienda v
       ...
       inner join estado_has_idioma ehi
                  on ehi.idEstado = rhe.idEstado and ehi.nombre='" . $_GET['estado'] . "' where idVendedor = :idV";
    $stm = DB::conn()->prepare($sql);
    $stm->bindValue(":idV", $idV);
} else {
    $sql = "select v.nombre   nomVivienda,
       p.nombre   nomPersona,
       ehi.nombre nomEstat,
       e.id       idEstat,
       v.id       idVivienda,
...
from vivienda v
       ...
       inner join estado_has_idioma ehi on e.id = ehi.idEstado where idVendedor= :idV";
    $stm = DB::conn()->prepare($sql);
    $stm->bindValue(":idV", $idV);
}
$stm->execute();
